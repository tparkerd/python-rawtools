...
from rawtools import convert, crop, generate, log, qualitycontrol, raw2img
...
def raw_convert():
    ...
    parser.add_argument(
        "--format",
        default="uint16",
        help=f"Desired output .RAW format. Supported formats: {supported_output_formats}",
    )
    parser.add_argument(
        "path",
        metavar="PATH",
        type=str,
        nargs="+",
        help=f"Input directory to process. Supported formats: {supported_input_formats}",
    )
    ...
    if args.format not in supported_output_formats:
        raise ValueError(
            f"Unsupported format, '{args.format}' specified. Please specify a supported format: {supported_output_formats}"
        )
...
def raw_generate():
    ...
    parser.add_argument(
        "path", metavar="PATH", type=str, nargs="+", help="Image filepath(s)"
    )
    args = parser.parse_args()
...
def raw_nsihdr():
    ...
    parser.add_argument(
        "--gui", action="store_true", default=False, help="(Experimental) Enable GUI"
    )
    parser.add_argument(
        "path", metavar="PATH", type=str, nargs="+", help="List of .nsihdr files"
    )
    args = parser.parse_args()
...
def raw_qc():
    ...
    parser.add_argument(
        ...
        type=int,
        default=argparse.SUPPRESS,
        help="Extract a slice from volume's side view. (default: floor(x/2))",
    )
    parser.add_argument(
        "--font-size",
        dest="font_size",
        ...
        type=int,
        default=24,
        help="Font size of labels of scale.",
    )
    parser.add_argument(
        "path",
        metavar="PATH",
...
def raw_image():
    ...
    parser.add_argument(
        "--format",
        default="png",
        help="Set image filetype. Availble options: ['png', 'tif']",
    )
    parser.add_argument(
...
def raw_crop_by_node():
    description = "Crop .raw files using slice indices provided in a CSV file"
    parser = argparse.ArgumentParser(
        description=description, formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--csv", required=True, help="File path to CSV with metadata about RAW files"
    )
    parser.add_argument(
        "-o",
        "--outdir",
        required=True,
        help="Destination directory for output cropped volumes",
    )
    parser.add_argument(
        "path", metavar="PATH", type=str, nargs=1, help="Search path for RAW files"
    )
    args = parser.parse_args()
    ...
    args.module_name = "crop-by-node"
    log.configure(args)

    # Make sure input CSV exists
    if not os.path.exists(args.csv):
        raise FileNotFoundError(args.csv)

    args.path = args.path[0]
    crop.crop_by_node(args)
