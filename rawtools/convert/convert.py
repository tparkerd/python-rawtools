...
def _infer_input_format(path: FilePath) -> bool:
    raise NotImplementedError


def convert(path: FilePath, *args, **kwargs) -> None:
    logging.debug(f'{path=}')
    logging.debug(f'{args=}')
    logging.debug(f'{kwargs=}')

    input_format = kwargs.get('_from', _infer_input_format(path))
    ...
    if not datasets:
        ...
        if input_format == 'raw':
            ...
            batch_convert(*raws, ext=output_format, bitdepth=kwargs.get('bitdepth'), **kwargs)
        else:
