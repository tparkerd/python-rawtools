...
def bitdepth(name):
    ...
    raise TypeError(
        f"bitdepth() argument must be a string, not '{type(name)}'",
    )
...
def determine_bit_depth(fp, dims):
    ...
    if file_size == minimum_size:
        ...
        if file_size < expected_filesize:
            logging.warning(
                f"Detected possible data corruption. File is smaller than expected '{fp}'. Expected at <{expected_filesize}> bytes but found <{file_size}> bytes. Defaulting to unsigned 16-bit.",
            )
            return 'uint16'
        else:
            logging.warning(
                f"Unable to determine bit-depth of volume '{fp}'. Expected at <{expected_filesize}> bytes but found <{file_size}> bytes. Defaulting to unsigned 16-bit.",
            )
            return 'uint16'
...
def __parse_resolution(line, dat_format):
    ...
    if match is not None:
        ...
        if not dims or len(dims) != 3:
            raise Exception(
                f"Unable to extract dimensions from DAT file. Found dimensions: '{dims}'.",
            )
        return dims
...
def __parse_slice_thickness(line, dat_format):
    ...
    if match is not None:
        ...
        if not dims or len(dims) != 3:
            raise Exception(
                f"Unable to extract slice thickness from DAT file: '{line}'. Found slice thickness: '{dims}'.",
            )
        return dims
...
def read(fp):
    ...
    with open(fp) as ifp:
        ...
        for line in ifp.readlines():
            ...
            if (
                object_filename := __parse_object_filename(line, dat_format)
            ) is not None:
