...
def process(fp, *args, **kwargs):
    ...
    for co in cutoffs:
        ...
        start = kwargs[co]
        # Collate information about subset volume
        subset_volumes[co] = {}
        ...
        logging.info(f"Export '{co}': {start} to {end}")
        logging.debug(pformat(subset_volumes[co]))
        dat.write(cropped_dat_ofpath, (x, y, end - start + 1), (xth, yth, zth))
        total_slices += end - start + 1

    # Open file handlers
    logging.info(f"Opening input data: '{fp}'")
        ...
        for i in range(earliest_included_slice, end + 1):
            ...
            for key in subset_volumes.keys():
                if subset_volumes[key]["start"] <= i <= subset_volumes[key]["end"]:
                    ...
                    chunk.tofile(subset_volumes[key]["ofp"])
                pbar.update(1)
    ...
    try:
        ...
        for uid in scan_uids:
            ...
            for root, dirs, files in os.walk(gargs.path):
                for fp in files:
                    if fp.endswith(".raw") and fp.split("_")[3] == uid:
