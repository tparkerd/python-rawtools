...
def img2pct(path, format="out", **kwargs):
    ...
    if format == "out":
        for y in track(range(len(files))):
            ...
            if y == 0:
                imgs = np.zeros(
                    (len(files), img.shape[0], img.shape[1]), np.uint8)
            imgs[y, ...] = img
        ...
        if dryrun:
            with open(output_fpath, "wb+") as ifp:
                ...
                np.savetxt(ifp, indices[..., (1, 2, 0)],
                           fmt='%s', delimiter=' ')
            logging.info(f"Created point-cloud data file: '{output_fpath}'")
        else:
            logging.info(f"Dry-run mode. Not generating files.")
...
def cli():
    ...
    parser.add_argument("--format", dest="fileformat",
                        default="out", choices=["out", "obj", "xyz"])
    parser.add_argument("path", metavar='PATH', type=str,
                        nargs='+', help='directory of binary image slices.')
...
def main():
    ...
    for fpath in path:
        img2pct(fpath, format=format, verbose=verbose,
